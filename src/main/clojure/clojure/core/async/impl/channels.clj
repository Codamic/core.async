      ...
      (when (.hasNext iter)
        (loop [^Lock putter (.next iter)]
          ...
          (let [put-cb (and (impl/active? putter) (impl/commit putter))
                executor (and (impl/active? putter) (impl/executor putter))]
            (.unlock putter)
            (when put-cb
            ...
            (let [put-cb (and (impl/active? handler) (impl/commit handler))]
              ...
              (if put-cb
                (let [done? (reduced? (add! buf val))]
                  (if (pos? (count buf))
                    (let [iter (.iterator takes)
                          ;; TODO: instead of a vector of take-cbs return a vector of cbs with executors
                          take-cbs (loop [takers []]
                                     (if (and (.hasNext iter) (pos? (count buf)))
                                       (let [^Lock taker (.next iter)]
                                         (.lock taker)
                                         (let [ret (and (impl/active? taker) (impl/commit taker))
                                               executor (and (impl/active? taker) (impl/executor taker))]
                                           (.unlock taker)
                                           (if ret
                                             (let [val (impl/remove! buf)]
                                               (.remove iter)
                                               (recur (conj takers [(fn [] (ret val)) executor])))
                                             (recur takers))))
                                       takers))]
                      (if (seq take-cbs)
                        (do
                          (when done?
                            (abort this))
                          (.unlock mutex)
                          (doseq [[f executor] take-cbs]
                            (dispatch/run f executor)))
                        (do
                          (when done?
                            (abort this))
                          (.unlock mutex))))
